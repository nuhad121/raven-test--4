...
const RavenAuth = {
  ...
  normalizePhone(phone) {
    return typeof RavenUI !== 'undefined' && RavenUI.normalizePhone
      ? RavenUI.normalizePhone(phone)
      : String(phone || '').replace(/\s/g, '');
  },

  phoneInUse(phone, users) {
    const norm = this.normalizePhone(phone);
    if (!norm) return false;
    return users.some((u) => this.normalizePhone(u.phone) === norm);
  },
  ...
  async register({ name, email, phone, password }) {
    ...
    if (!phoneNorm || (typeof RavenUI !== 'undefined' && !RavenUI.validatePhone(phoneNorm))) {
      return { ok: false, message: 'Enter a valid Bangladesh phone (01XXXXXXXXX).' };
    }

    if (db.users.some((u) => u.email.toLowerCase() === emailLower)) {
      return { ok: false, message: 'Email already registered.' };
    }
    if (this.phoneInUse(phoneNorm, db.users)) {
      return { ok: false, message: 'This phone number is already registered. One account per number.' };
    }

    if (typeof RavenCloudUsers !== 'undefined' && RavenCloudUsers.isEnabled()) {
      const emailTaken = await RavenCloudUsers.emailExistsInCloud(emailLower);
      if (emailTaken) return { ok: false, message: 'Email already registered on another device.' };
      const phoneTaken = await RavenCloudUsers.phoneExistsInCloud(phoneNorm);
      if (phoneTaken) {
        return { ok: false, message: 'This phone number is already registered. One account per number.' };
      }
    }
