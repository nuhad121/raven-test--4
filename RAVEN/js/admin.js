  ...
  async renderUsers() {
    ...
    if (typeof RavenCloudUsers !== 'undefined' && RavenCloudUsers.isEnabled()) {
      ...
      if (remote.ok) {
        ...
        const msg = remote.message || 'unavailable';
        const needsTable = /raven_users|schema cache/i.test(msg);
        syncMsg = needsTable
          ? '<div class="alert alert-danger py-2 mb-0"><strong>Supabase table missing</strong><br>1) Open <a href="https://supabase.com/dashboard/project/unibhpcybywpffntuvxd/sql/new" target="_blank" rel="noopener">SQL Editor</a><br>2) Copy all code from <code>supabase/schema.sql</code> in your project<br>3) Click <strong>Run</strong> → then Refresh this page</div>'
          : '<span class="text-danger"><i class="bi bi-x-circle"></i> Cloud error: ' +
            RavenUI.escapeHtml(msg) +
            ' — see <code>SUPABASE-SETUP.md</code></span>';
      }
    } else {
      syncMsg =
